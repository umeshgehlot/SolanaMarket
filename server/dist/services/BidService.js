...
const MOCK_BIDS = [
    {
        ...
        nftId: new mongoose_1.default.Types.ObjectId('60d21b4667d0d8992e610c85'),
        bidder: new mongoose_1.default.Types.ObjectId('60d21b4667d0d8992e610c86'),
        price: 2.5,
        status: models_1.BidStatus.ACTIVE,
    ...
    },
    {
        _id: new mongoose_1.default.Types.ObjectId(),
        nftId: new mongoose_1.default.Types.ObjectId('60d21b4667d0d8992e610c85'),
        bidder: new mongoose_1.default.Types.ObjectId('60d21b4667d0d8992e610c87'),
        price: 3.0,
        status: models_1.BidStatus.ACTIVE,
...
class BidService {
     ...
     */
    async getBidsForNFT(nftId, status) {
        try {
            // Using mock data instead of MongoDB
            logger_1.logger.info(`Getting bids for NFT ${nftId} with status ${status}`);
            ...
            if (status) {
                ...
                filteredBids = filteredBids.filter(bid => bid.status === models_1.BidStatus.ACTIVE);
            }
            ...
            logger_1.logger.error('Error getting bids for NFT:', error);
            throw error;
        }
    }
    /**
        ...
        try {
            ...
            return {
                ...
                nftId: {
                    ...
                    owner: new mongoose_1.default.Types.ObjectId('60d21b4667d0d8992e610c88')
                }
            };
        }
        catch (error) {
            logger_1.logger.error('Error getting bid:', error);
            throw error;
        }
    }
    /**
     ...
     */
    async placeBid(nftId, bidderWalletAddress, price, expirationDays) {
        try {
            logger_1.logger.info(`Placing bid on NFT ${nftId} by ${bidderWalletAddress} for ${price} SOL with ${expirationDays} days expiration`);
            // Mock NFT data
            const mockNftId = new mongoose_1.default.Types.ObjectId(nftId);
            // Mock bidder data
            const mockBidderId = new mongoose_1.default.Types.ObjectId();
            ...
            logger_1.logger.info(`Created transaction record for bid ${newBid._id}`);
     ...
     * Accept a bid
     */
    async acceptBid(bidId, sellerWalletAddress) {
        try {
            logger_1.logger.info(`Accepting bid ${bidId} by seller ${sellerWalletAddress}`);
            ...
            this.bids[bidIndex].status = models_1.BidStatus.ACCEPTED;
            this.bids[bidIndex].updatedAt = new Date();
            // Log NFT ownership transfer
            logger_1.logger.info(`NFT ${mockNftId} ownership transferred to ${bid.bidder}`);
            logger_1.logger.info(`Bid ${bidId} accepted successfully`);
            // Log transaction creation (instead of creating in DB)
            logger_1.logger.info(`Created transaction record for accepted bid ${bidId}`);
     ...
     * Cancel a bid
     */
    async cancelBid(bidId, bidderWalletAddress) {
        try {
            logger_1.logger.info(`Cancelling bid ${bidId} by bidder ${bidderWalletAddress}`);
            // Find the bid in mock data
            const bidIndex = this.bids.findIndex(b => b._id.toString() === bidId);
            if (bidIndex === -1) {
                throw new Error(`Bid with ID ${bidId} not found`);
            ...
            logger_1.logger.info(`Verified bidder ${bidderWalletAddress} is authorized to cancel bid ${bidId}`);
            // In a real implementation, this would interact with the Solana blockchain
            // to cancel the bid and return the funds from escrow
            const signature = 'mock-signature-' + Date.now();
            // Update the bid status in mock data
            this.bids[bidIndex].status = models_1.BidStatus.CANCELLED;
            this.bids[bidIndex].updatedAt = new Date();
            // Log transaction creation (instead of creating in DB)
            logger_1.logger.info(`Created transaction record for cancelled bid ${bidId}`);
            // Return the updated bid
            return {
                ...this.bids[bidIndex],
                bidder: {
     ...
     * Update expired bids
     */
    async updateExpiredBids() {
        try {
            const now = new Date();
            // Update expired bids in mock data
            this.bids.forEach(bid => {
                if (bid.status === models_1.BidStatus.ACTIVE && bid.expiresAt < now) {
                    bid.status = models_1.BidStatus.EXPIRED;
                }
            });
            logger_1.logger.info('Updated expired bids in mock data');
        }
        catch (error) {
            logger_1.logger.error('Error updating expired bids:', error);
        }
    }
}
