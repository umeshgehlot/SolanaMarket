...
const MOCK_USERS = [
    {
        _id: new mongoose_1.default.Types.ObjectId('60d21b4667d0d8992e610c86'),
        username: 'user1',
        walletAddress: 'wallet-address-1',
        avatar: 'https://via.placeholder.com/150'
    },
    {
        _id: new mongoose_1.default.Types.ObjectId('60d21b4667d0d8992e610c87'),
        username: 'user2',
        walletAddress: 'wallet-address-2',
        avatar: 'https://via.placeholder.com/150'
    },
    {
        _id: new mongoose_1.default.Types.ObjectId('60d21b4667d0d8992e610c88'),
        username: 'user3',
...
const MOCK_NFTS = [
    {
        _id: new mongoose_1.default.Types.ObjectId('60d21b4667d0d8992e610c85'),
    ...
    {
        _id: new mongoose_1.default.Types.ObjectId('60d21b4667d0d8992e610c89'),
    ...
    {
        _id: new mongoose_1.default.Types.ObjectId('60d21b4667d0d8992e610c90'),
...
class NFTService {
    ...
    async getNFTs(filters = {}) {
        try {
            logger_1.logger.info(`Getting NFTs with filters: ${JSON.stringify(filters)}`);
            // Start with all NFTs
            ...
            if (filters.search) {
                ...
                filteredNFTs = filteredNFTs.filter(nft => nft.name.toLowerCase().includes(searchTerm) ||
            ...
            logger_1.logger.error('Error getting NFTs:', error);
            throw error;
        }
    }
    /**
    ...
    async getNFT(id) {
        try {
            logger_1.logger.info(`Getting NFT with ID: ${id}`);
            ...
            logger_1.logger.error('Error getting NFT:', error);
            throw error;
        }
    }
    /**
    ...
    async getNFTByMintAddress(mintAddress) {
        try {
            logger_1.logger.info(`Getting NFT with mint address: ${mintAddress}`);
            ...
            logger_1.logger.error('Error getting NFT by mint address:', error);
            throw error;
        }
    }
    /**
     * Create a new NFT
     */
    async createNFT(creatorWalletAddress, name, description, image, mintAddress, collectionId, royaltyPercentage, metadata) {
        try {
            logger_1.logger.info(`Creating NFT: ${name} by ${creatorWalletAddress}`);
            ...
            if (!creator) {
                ...
                logger_1.logger.info(`Created new user with wallet address: ${creatorWalletAddress}`);
            }
            ...
            if (collectionId) {
                logger_1.logger.info(`Collection ${collectionId} associated with NFT`);
            ...
            logger_1.logger.info(`NFT created with ID: ${newNFT._id}`);
            logger_1.logger.info(`Mock transaction signature: ${signature}`);
     ...
     */
    async listNFT(nftId, ownerWalletAddress, price) {
        try {
            logger_1.logger.info(`Listing NFT ${nftId} for sale by ${ownerWalletAddress} at price ${price}`);
            ...
            logger_1.logger.info(`Created transaction record for listing NFT ${nftId}`);
            logger_1.logger.info(`Mock transaction signature: ${signature}`);
     ...
     * Unlist an NFT
     */
    async unlistNFT(nftId, ownerWalletAddress) {
        try {
            logger_1.logger.info(`Unlisting NFT ${nftId} by ${ownerWalletAddress}`);
            // Find the NFT in mock data
            const nftIndex = this.nfts.findIndex(nft => nft._id.toString() === nftId);
            if (nftIndex === -1) {
                throw new Error(`NFT with ID ${nftId} not found`);
            ...
            this.nfts[nftIndex].updatedAt = new Date();
            // Log transaction creation (instead of creating in DB)
            logger_1.logger.info(`Created transaction record for unlisting NFT ${nftId}`);
            logger_1.logger.info(`Mock transaction signature: ${signature}`);
            // Return the updated NFT with populated data
            return {
                ...this.nfts[nftIndex],
     ...
     * Buy an NFT
     */
    async buyNFT(nftId, buyerWalletAddress) {
        try {
            // Find the NFT
            ...
            let buyer = await models_1.User.findOne({ walletAddress: buyerWalletAddress });
            if (!buyer) {
                buyer = await models_1.User.create({ walletAddress: buyerWalletAddress });
            }
            // Find the current owner
            const owner = await models_1.User.findById(nft.owner);
            if (!owner) {
            ...
            const transaction = await models_1.Transaction.create({
                type: models_1.TransactionType.SALE,
                nft: nft._id,
                from: owner._id,
                to: buyer._id,
                price,
     ...
     */
    async transferNFT(nftId, fromWalletAddress, toWalletAddress) {
        try {
            // Find the NFT
            const nft = await models_1.NFT.findById(nftId);
            if (!nft) {
            ...
            let toUser = await models_1.User.findOne({ walletAddress: toWalletAddress });
            if (!toUser) {
                toUser = await models_1.User.create({ walletAddress: toWalletAddress });
            }
            // In a real implementation, this would interact with the Solana blockchain
            // to transfer the NFT
            const signature = 'mock-signature-' + Date.now();
            ...
            // Create a transaction record
            const transaction = await models_1.Transaction.create({
                type: models_1.TransactionType.TRANSFER,
                nft: nft._id,
                from: fromUser._id,
                to: toUser._id,
                signature,
...
exports.default = new NFTService();
